            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('inheritance.index') }}">Inheritance Certificate</a></li>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card card-info">
                        ...
                        <!-- /.card-header -->
                        <form id="inheritanceCertificateForm" method="post">
                            @csrf
                            @method('PUT')
                            <div class="card-body">
                                <div class="row">
                                                ...
                                                @if (count($users))
                                                    @foreach ($users as $user)
                                                        @if (isset($user->people->approved_id))
                                                            <option value="{{$user->system_id}}" {{ $inheritance->user_id == $user->id ? 'selected' : '' }}>{{$user->people->approved_id}} - {{$user->name}} - {{$user->mobile}}</option>
                                                        @endif
                                                    @endforeach
                                                @else
                                    ...
                                    <div class="col-md-4 text-right">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control form-control-sm d-inline-block" name="death_certificate_id" id="death_certificate_id" style="width: 150px;" value="{{ $inheritance->deathPerson ? $inheritance->deathPerson->system_id : '' }}">
                                            <br>
                                    ...
                                    <table class="table table-bordered">
                                        <thead>
                                            <tr>
                                                <td colspan="6"> Add Inheritance Members Info. নোট- সনদটি বাংলায় নিতে চাইলে সব তথ্য বাংলায় দিন আর সনদটি ইংরেজীতে নিতে চাইলে ইংরেজীতে দিন।</td>
                                            </tr>
                                            <tr>
                                                <td>#SL</td>
                                                <td>Name</td>
                                        ...
                                        <tbody id="membersTbody">
                                            @php
                                                $members = $inheritance->members ? json_decode($inheritance->members, true) : [];
                                                $sl = 1;
                                            @endphp
                                            @if(count($members) > 0)
                            ...
                            <div class="card-footer">
                                <div class="form-group row">
                                    <a href="{{route('inheritance.index')}}" class="btn btn-default float-right">Cancel</a>
                                    <div class="col-sm-9">
...
@push('script')
    <script>
        $(document).ready(function() {
            $(".select2").select2();
            $("#inheritanceCertificateForm").on('submit', function(e) {
                e.preventDefault();
                let thisForm = $(this);
                $.ajax({
                    type: "POST",
                    url: "{{ route('inheritance.update', $inheritance->id) }}",
