...
@endif
    ...
    <strong>
        ...
        {{ $certificate->user->people->professionSubCategory->bn_name ?? '' }}
        {{ $certificate->user->people->professionType->bn_name ?? '' }}
    </strong> এবং তার মাসিক আয় আনুমানিক   
    <strong>{{ bnValue(number_format($certificate->monthly_income ?? 0,2)) }}</strong> টাকা 
    এবং বার্ষিক আয় 
    <strong>{{ bnValue(number_format($certificate->yearly_income ?? 0,2)) }}</strong> টাকা।
