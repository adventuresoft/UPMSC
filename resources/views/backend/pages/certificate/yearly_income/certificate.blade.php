...
His/Her declared statement of annual income is certified to be true. To my knowledge, he is of good character and has not been involved in any crime against law and order or the state.
                        </p>
                        
                        <p style="margin-left:40px;">
    His monthly income is estimated to be
    <strong>{{ number_format($certificate->monthly_income ?? 0,2) }}</strong>  BDT
and annual income
    <strong>{{ number_format($certificate->yearly_income ?? 0,2) }}</strong>  BDT
</p>
<p style="margin-left:40px;">
    Occupation: 
    <strong>
        {{ $certificate->user->people->profession->en_name ?? '' }}
        {{ $certificate->user->people->professionCategory->en_name ?? '' }}
        {{ $certificate->user->people->professionSubCategory->en_name ?? '' }}
        {{ $certificate->user->people->professionType->en_name ?? '' }}
    </strong>
</p>
...
<script>
    function goToIndex(){
        // Redirect to Citizen Certificate index page
